...
**Difficulty:** Easy  
**Topics:** `Array`, `HashMap`, `Sliding Window`, `Sorting`   
...
3. Sum up all elements (frequency × value) of those top `x` elements.
...
1. [1,1,2] → freq = {1:2, 2:1} → top 2 → sum = 1×2 + 2×1 = 4  
2. [1,2,2] → freq = {1:1, 2:2} → top 2 → sum = 1×1 + 2×2 = 5  
3. [2,2,3] → freq = {2:2, 3:1} → sum = 2×2 + 3×1 = 7  
4. [2,3,3] → freq = {2:1, 3:2} → sum = 2×1 + 3×2 = 8  
...
## ⚡ Approach 2 – Sliding Window + Frequency Array (Optimized)
...
- Pick top `x` entries and add `(frequency × value)` for each.

This ensures we always pick the correct elements by the problem’s definition.
...
- **Time:** `O((n−k+1) × 50 log 50)` → effectively `O(n)`
- **Space:** `O(50)` → constant

### ✅ Pros

- Correctly respects “frequency-first, value-second” ranking.
...
|-----------|-------------|------|--------|--------|
| Brute Force | Build & sort new map for each window | O((n−k+1) × k log k) | O(k) | Clear but redundant |
| Sliding Window | Maintain frequency array, O(1) updates | O(n × 50) ≈ O(n) | O(50) | Best for small value range |

---

✅ **Key takeaway:**  
When values are bounded (`≤50`), replace heavy sorting with a fixed-size frequency array + sliding window.  
That’s the real optimisation trick here.
