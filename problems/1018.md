...
**Difficulty:** Easy  
**Topics:** `Array`, `Math`, `Bit Manipulation`, `Prefix Sum`  
...
## ⚠️ What Was Wrong in My Original Solution
...
- Once `num` is inexact, `num%5` is no longer reliable → some prefixes that are divisible by 5 look like they are not (and vice versa).
...
## 🍃 Correct Approach – Track Modulo Only

### Idea

We never need the full integer `x_i` — we only need `x_i % 5`.
